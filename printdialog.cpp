...
{
    auto mainwindowUi=mainwindow->ui;
    auto instance=DataManager::GetInstance();

    auto pdfFile=QFileDialog::getSaveFileName(this,"保存PDF","./","*.pdf");
...

    {
        ...
        {
            QVector<QString> tmp;
            auto rec = instance->getChannelInfo("InputChannel");
            tmp << QString::number(i);
            tmp << rec[i][0] << rec[i][1] << rec[i][2] << rec[i][3];
            rec = instance->getChannelInfo("Flur");
            QString stopRate = QString("%1,%2,%3,%4").arg(rec[i][0]).
                arg(rec[i][1]).arg(rec[i][2]).arg(rec[i][3]);
            tmp << stopRate;
            QString close = QString("关,0.7");
            tmp << close;
            rec = instance->getChannelInfo("callibrate");
            tmp << QString{ "%1,%2" }.arg(rec[0][0], rec[0][1]);
            table << tmp;
        ...
        QPixmap grabed{ mainwindowUi->graphicsView->grab() };
        writer.paintBoltText(QRectF{ 500,8500,2000,500 }, "试验窗口图片", Qt::AlignLeft);
        writer.paintPixmap(QRect{ 500,9000,8500,4500 }, grabed);
...
void PrintDialog::on_pushButton_2_clicked()
{
    close();
}
